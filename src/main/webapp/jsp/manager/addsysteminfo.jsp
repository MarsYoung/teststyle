...
<body>
	...
	没有记录可显示！
				...
				<pg:pager items="${pager.maxElements }" url="${pager.url }" maxIndexPages="${pager.maxIndexPages }"
					maxPageItems="${pager.pageSize }">
					<pg:index>
						<pg:first>
							<a href="javascript:void(0)" onclick="changeSystemInfoPage('${pageUrl }')" style="color: green">首页</a>
						</pg:first>
						<pg:prev>
							<a href="javascript:void(0)" onclick="changeSystemInfoPage('${pageUrl }' )" style="color: green">上一页</a>
						</pg:prev>
						<pg:pages>
							<c:choose>
								<c:when test="${pager.pageNumber eq pageNumber}">
									<a href="javascript:void(0)" onclick="changeSystemInfoPage('${pageUrl }')" style="color: white">${pageNumber
										}</a>
								</c:when>
								<c:otherwise>
									<a href="javascript:void(0)" onclick="changeSystemInfoPage('${pageUrl }')" style="color: green">${pageNumber
										}</a>
								</c:otherwise>
							</c:choose>
						</pg:pages>
						<pg:next>
							<a href="javascript:void(0)" onclick="changeSystemInfoPage('${pageUrl }')" style="color: green">下一页</a>
						</pg:next>
						<pg:last>
							<a href="javascript:void(0)" onclick="changeSystemInfoPage('${pageUrl }')" style="color: green">末页</a>
						</pg:last>
					</pg:index>
				</pg:pager>
